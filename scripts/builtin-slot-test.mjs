...
const tempRoot = mkdtempSync(join(tmpdir(), 'babylon-builtin-slot-test-'));
const outDir = join(tempRoot, 'out');
const tsconfigPath = join(tempRoot, 'tsconfig.builtin-slot.json');
  ...
  JSON.stringify(
    {
      ...
      include: [
        ...
        normalizedProjectRoot + '/tests/editor/builtInSlotAssetSync.test.ts',
      ],
    },
    null,
    2,
...
try {
  ...
  run('node', ['--test', join(outDir, 'tests/editor/builtInSlotAssetSync.test.js')], projectRoot);
} finally {
