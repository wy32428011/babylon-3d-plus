...
const root = path.resolve('output/environment-building-effect/viewer');
...
try {
  ...
  await page.waitForFunction(() => window.viewerEngineStore?.Instances.some(engine => engine.scenes.some(scene => scene.meshes.some(mesh => mesh.metadata?.editorEnvironmentMesh && mesh.material?.name.endsWith('_effect') && mesh.isReady(true)))), null, { timeout: 120000 });
  await page.evaluate(() => {
    const scene = window.viewerEngineStore.Instances.flatMap(engine=>engine.scenes).find(scene=>scene.meshes.some(mesh=>mesh.metadata?.editorEnvironmentMesh && mesh.material?.name.endsWith('_effect')));
    ...
    if(count!==1)throw Error('发布特效实例数量不符: '+count);
  });
  const pixels = async () => {
  ...
  assert.ok(second.count > 500, '发布 Viewer 环境渐变保持可见');
  const state = await page.evaluate(() => {
    const engine = window.viewerEngineStore.Instances.find(engine => engine.scenes.some(scene => scene.meshes.some(mesh => mesh.metadata?.editorEnvironmentMesh && mesh.material?.name.endsWith('_effect'))));
    const mesh = engine.scenes.flatMap(scene => scene.meshes).find(mesh => mesh.metadata?.editorEnvironmentMesh && mesh.material?.name.endsWith('_effect'));
  ...
  assert.ok(state.vertices >= 8); assert.ok(state.indices >= 36);
  assert.equal(state.material, 'PBRMaterial'); assert.deepEqual(errors, []);
  ...
  console.log('PASS: 实际 DIST Viewer 加载环境 GLB、绑定建筑效果、PBR Shader 编译及可见像素。');
