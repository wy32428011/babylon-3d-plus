...
let movement = 1, fault = false, publishEnabled = true, sequence = 0;
...
try {
  ...
  await panel.getByLabel('启用表面箭头', { exact: true }).check();
  ...
  async function capture(name) {
    ...
    return page.evaluate(async data => {
      ...
      const pixels = ctx.getImageData(0, 0, canvas.width, canvas.height).data; let cyan = 0;
      for (let i = 0; i < pixels.length; i += 4) if (pixels[i + 1] > 90 && pixels[i + 1] > pixels[i] * 1.4 && pixels[i + 2] > pixels[i] * 1.6) cyan++;
      return { cyan };
    }, png.toString('base64'));
  }
  ...
  await page.waitForFunction(() => window.conveyorArrowHarness.visual().every(v => !v.enabled));
  assert.match(await panel.getByRole('status').textContent(), /未找到/);
  ...
  console.log('PASS: Editor 属性手势 / 正反停 WebGL / 保存重开 / 本地 MQTT 正反停故障过期恢复 / 退出预览');
} catch (error) {
