...
货物生成器的共享模板默认是内置立方体。选中生成器后，可从模型库把真实托盘、料箱或纸箱模型拖入“共享生成模板”替换默认立方体，仓储流程和三设备绑定不变。
...
1004、DDJ2、1005 模型实体均保存显式 `telemetryBinding`，模型生成器中的 `warehouseFlow` 再引用三条稳定 binding ID。任一引用缺失、设备类型错误或同一完整主键命中多个模型时，仓储流停止驱动，不按模型名称猜测。
...
1. 1004 `front_has_goods=true`：生成器创建共享模板实例并锁定 `containerCode`；相同真实条码若已在库位或出库流程中，直接拒绝重复生成。
...
7. `command=5` 会锁存“放货完成”事件；即使生成模型仍在异步加载或后续命令复位，运行时仍会重试登记，成功后把当前输出脱离生成器根节点并固定在 locator。
...
- 一个生成器同一时刻最多处理一件入库和一件出库；已入库货物可同时保存在多个 locator。
...
- 需要多条独立入库线时，应复制模型生成器并配置独立的三设备绑定。
...
- 停止运行预览、删除生成器、切换场景或销毁 SceneRuntime 时，统一释放活动输出、已入库货物根节点、模型容器、材质、贴图和脚本。
...
5. 1004 是否出现 `front_has_goods=true`，生成器是否配置共享模板。
