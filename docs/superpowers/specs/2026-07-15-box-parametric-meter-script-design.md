...
该位置同比补偿与其它通用模型脚本采用的“缩放前后读取实体根米空间底部中心并回补”语义等价。二者都用于隔离 `contentRoot` 的源单位缩放和底部中心归一化补偿，避免 GLB 内部存在大坐标或嵌套父级变换时，参数缩放把模型整体推离实体原点。
...
- `npm run smoke:model-parameters` 验证 Box 与其余 11 个模型的默认尺寸、参数变化、旋转不变、非均匀 scale、源包/副本同步、隐藏/禁用 helper 隔离和异常大尺寸防漂移；该命令已接入 `npm run smoke:units`。
