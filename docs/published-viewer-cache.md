...
```powershell
node --experimental-strip-types --test --test-concurrency=1 tests/digitalTwin/publishedAssetCache.test.ts tests/digitalTwin/publishedCacheStore.test.ts tests/runtime/skyboxTextureLoad.test.ts tests/runtime/skyboxContentHash.test.ts
npm run typecheck
