...
7. `front_movement_z=1/3` 时把货叉目标设为根据投影距离判断的行程，伸出方向由当前货格几何决定（不区分 1/3 的左右编码）；
8. `front_movement_z=2/4` 时把货叉目标设为 `0`；
...
| `front_movement_z` | 0 / 1、3 / 2、4 | 静止 / 前叉伸出 / 前叉收回 |
| `back_movement_z` | 0 / 1、3 / 2、4 | 静止 / 后叉伸出 / 后叉收回 |

> 伸出方向由当前货格几何决定（始终朝目标货格一侧），不区分 1/3 的左右编码；2/4 均为收回。
