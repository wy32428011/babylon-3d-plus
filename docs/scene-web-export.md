...
普通 Web 部署由 `viewer.showStatusOverlay` 决定运行状态层的初始可见性，并继续按场景内容显示自动巡检面板及手动漫游面板。数字孪生发布包（`version: 2`）在场景就绪后固定默认隐藏运行状态层、自动巡检浮窗和手动漫游浮窗；两个运行浮窗只能由大屏组件对应按钮发送的 `startAutoPatrol` / `startManualRoam` 命令打开，并保持互斥显示。同时按下鼠标左键和右键仅切换运行状态层。场景就绪后的运行状态层以 1 秒间隔显示 Babylon 实时 FPS；加载、启动阻断和真实运行异常不受隐藏状态影响。
