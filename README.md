...
- Hierarchy 选择与分组：支持在层级面板中选择场景对象，并与 Scene View 高亮状态同步；文件夹允许不限层级地包含子文件夹和普通对象，选中文件夹时会在 Scene View 递归高亮其全部可显示模型。左侧 Hierarchy 提供搜索、全部展开、全部收缩、新建根/子文件夹、单选/多选拖入任意层级文件夹、拖回根层级，以及实体/文件夹级显示隐藏、锁定解锁控制；拖拽会阻止把文件夹放入自身后代。
