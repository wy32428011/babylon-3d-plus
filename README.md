...
- 2026-09-20：货架库存 fetch 同步新增定时全量触发：工具栏“配置 Fetch 请求”可设同步间隔（毫秒，默认 60000，填 0 表示只在进入运行预览时同步一次），编辑器运行预览与数字孪生发布包 Viewer 都按该间隔持续拉取，覆盖外部改库存的盲区。定时 tick 遇到在途请求（如放货单排同步）会跳过本轮，避免抢走响应代际导致格口抑制不解除、停留货箱不销毁；定时失败提示只打印一次，链路恢复后再失败会重新提示。间隔为场景配置项，随场景保存并进入发布包。
