...
- 主动同步替换与参数保留：模型库和环境库中的“同步模型库”会等待普通、组合和环境全库同步，再将新版资源应用到当前场景；“同步场景模型”用于定向更新，本地场景也可使用。替换保留实体 ID、摆放、资产编号和业务绑定。参数定义、默认值、范围、绑定及执行规则全量使用新版：新增参数使用新版默认，删除参数及其旧值直接移除，同 key 保留场景显式保存值（包括 0、false、空串和向量）；未保存值使用新版默认。同 key 类型/范围变化不重置数值，差异只写日志，不弹确认或阻止模型替换与发布。不能执行的声明式参数绑定/规则或参数纹理仅隔离对应效果，后续绑定继续。保存重开不再按新范围清洗历史值；用户主动编辑时只校验实际改动字段。下载期间场景变化会重新校验最新配置，连续变化则保留编辑并提示重试；场景替换支持一次撤销/重做，并等待脚本与真实首帧后结束加载。已同步资源按固定版本引用，保存后保留到场景文件，SOURCE/DIST 继续使用同一份配置。未绑定场景缺少可确认的中台来源身份时保留原资源；无关模型库下载问题单独提示，发布会自动重新同步并检查当前场景资源问题。
