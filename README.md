...
- 2026-08-28：发布后的数字孪生 Viewer 默认隐藏右下角自动巡检面板；同时按下鼠标左键和右键时，与运行状态层一起切换显示或隐藏，运行状态层新增每秒刷新的 Babylon FPS 数值。普通独立 Web 部署和编辑器运行预览保持原有显示行为。
