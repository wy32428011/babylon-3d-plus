...
- Project 资源库外观：底部 Project 面板已切换为资源库浏览器样式，并将图库区域加高到约 `300px` 至 `460px` 自适应，包含模型库、POI库、主题库、组合库、环境库、图表库、图片库七个页签，以及筛选占位行和可换行资源卡片；模型库卡片使用深色直角卡、上方缩略图、下方两行居中文字和单行省略标题，模型库内置立方体、球体、地面、虚拟定位线框、半球光、方向光、点光源七类基础资源，并支持导入普通模型文件夹展示项目内模型卡片；环境库使用独立环境模型导入入口，支持点击应用或拖入右侧环境属性区。
- 模型文件夹导入：普通模型与环境模型严格分库，导入入口决定资产分类；模型库导入会将有效模型包复制到当前项目目录的 `Assets/Models` 下，环境库导入会复制到 `Assets/Environments` 下，并通过 `.babylon-editor/asset-index.json` v2 在下次打开项目时分别恢复。v1 旧索引条目默认归入模型库，且不会移动旧文件。同名包重新导入只覆盖当前入口对应的目标库，不会跨库覆盖。模型库卡片只能拖入 Scene 创建实体；环境库卡片只能点击应用，或拖入 Inspector 的环境属性区。导入会扫描一级模型包中的 `.glb/.gltf`、读取 `meta.json` 展示名称、`lengthUnit` 源单位与可选封面图；导入模型最终按米进入场景；`meta.json.lengthUnit` 支持 `meter`/`m`、`centimeter`/`cm`、`millimeter`/`mm`，`meta.json.thumbnail` 或 `meta.json.cover` 可指向模型包内 `.png/.jpg/.jpeg/.webp` 相对路径作为卡片缩略图；缺失 `lengthUnit` 时会优先根据 GLB 包围盒与 `meta.json` 中的长宽高等尺寸参数推断米/毫米/厘米，无法推断时默认按 `meter` 处理，不支持的显式单位会跳过该模型包。
...
- `sceneSettings.environment` 只记录环境模型包路径、缩略图、当前激活变体和包内变体列表；字段结构保持不变，环境模型包内 `meta.json` 格式也保持不变。运行时加载的环境底座不进入 Hierarchy、不参与拾取和 Gizmo，也不会影响实体保存。
