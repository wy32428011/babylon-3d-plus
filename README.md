...
- 数据中台场景最新模型：从首页打开数据中台工程时，按场景实际引用的普通模型、组合模型和环境模型查询最新资源，模型生成器、报警模板、漫游人物及点击设备类型等间接引用一并检查。模型资源按版本缓存，旧文件保留；兼容更新以一次可撤销操作应用，保留各实例的位置、旋转、缩放、资产编号、参数值和业务绑定，仅为新参数补充默认值。删除参数、类型/单位/绑定变化、数值越界、缺失节点或贴图会明确阻断，不能重置后继续。已激活模型的脚本初始化失败或首帧超时会回退本轮场景更新；未触发的动态模板仍需在实际生成时验证脚本行为。编辑过程中可使用资源面板的“同步场景模型”执行相同更新；有变化后需要保存，发布使用保存后的固定资源。历史 SOURCE 编辑打开会采用最新兼容模型，已发布 Viewer 保留发布快照。可用远端版本且缓存完整时不重复下载，旧接口缺少版本信息时重新下载核对；无可获取附件的外部 glTF/相对贴图会按资源不完整阻断。旧版本缓存暂不自动淘汰。定向回归运行 `npm run test:scene-model-update`，实际 Electron/HTTP/SOURCE/DIST/Viewer 验证运行 `npm run smoke:scene-model-update`。
...
- 本地场景资源同步：从“打开场景文件”或“最近场景”打开时，会等待当前配置的数据中台完成普通/组合模型同步，并按稳定 ID、唯一名称匹配场景环境（兼容原来源或修订不同）。只用当前中台候选重新关联匹配模型，即使共享缓存未变化也会更新场景引用，再等待环境、设备和首帧实际就绪。环境仅下载场景匹配项；未匹配的普通模型保留原引用，环境匹配歧义或必需资源不可用时显示错误并支持重新同步、取消返回首页。未配置数据中台时使用本地资源；从数据中台 SOURCE 入口打开按场景最新模型策略更新兼容资源。
