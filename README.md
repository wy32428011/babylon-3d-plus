...
- 发布 Viewer 版本缓存：首次加载把场景、资源清单、模型、外链纹理、脚本及天空盒文件存入 IndexedDB；同版本刷新复用文件，并复用 Draco、Meshopt 几何解码及 KTX2 纹理转码结果，HDR/受支持 EXR 沿用已有天空盒解码缓存。每次发布生成独立 `cacheRevision`，刷新先读取最新配置，新版本自动重新加载；下载期间发生发布变更会提示重新加载，避免混用版本。项目运行配置、启停、业务接口与 MQTT 仍实时读取。浏览器刷新仍需重建场景、运行脚本、解析普通 PNG/JPEG、上传 GPU 及预过滤，不能保证零初始化。缓存按部署地址、发布标识、解码参数和引擎版本隔离，KTX2 额外区分显卡能力；资源与模型/纹理解码缓存总计上限 1 GiB、4096 条，单条超过 512 MiB 不缓存，达到限额时淘汰较久未使用项。浏览器清理存储、空间不足或禁用 IndexedDB 时自动恢复正常加载。普通 HTTP 内网可用，无需 Service Worker；旧包没有版本标识时保持原行为，需要使用新版编辑器重新发布。验证命令和边界见 [发布 Viewer 缓存](docs/published-viewer-cache.md)。
