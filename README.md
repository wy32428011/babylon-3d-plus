...
- 场景连续加载与全库同步：一次打开会持续显示同一个加载层，直到本轮资源更新、设备/环境初始化和实际首帧完成；资源加载失败或超时不会标记为 100%，可重试或显式选择“保留资源问题并继续编辑”。场景准备结束后自动后台同步中台全部普通、组合和环境模型，包括未绑定模型；模型库/环境库也提供“同步模型库”入口。后台同步刷新卡片和共享缓存，当前场景的固定版本仍由“同步场景模型”更新。右上角资源问题提示可关闭并从“资源问题 · 查看详情”重新展开；模型/环境同步提示支持关闭，成功提示在非悬停、非键盘操作时自动收起。关闭提示不清除资源问题或发布校验。
