...
- Transform Gizmo：Scene View 中支持移动、旋转、缩放三种可视化操控模式，普通拖拽结束后写入撤销/重做历史；编辑模式下选中单个未锁定普通导入模型并使用移动工具时，可按住 `Shift` 拖动 X/Y/Z 单轴箭头进入模型阵列。
...
- Shift 拖拽阵列：保持编辑模式和移动工具，单选一个未锁定的普通导入模型，按住 `Shift` 后拖动 X/Y/Z 单轴箭头；拖动超过当前轴向模型跨度的一半后开始按实际投影尺寸实时出现零间距副本，最多 100 个，原模型始终不移动。松开左键后方向固定为本次 `+X/-X/+Y/-Y/+Z/-Z（局部/世界）`，可在弹框修改新增副本数量、净间距和一次性编号规则，预览会实时更新；`Esc`、点击遮罩或“取消”不会写入场景和撤销历史。
- 切换坐标空间：点击 `局部` 或 `全局`。
