...
- Babylon Scene View：在 Scene 面板中渲染 Babylon.js 3D 场景，并同步当前场景文档中的基础 Mesh、导入模型与灯光；默认编辑器相机使用 `标准` 视野，并可在 Toolbar 中切换 `近景`、`标准`、`远景`、`全景` 四档可视范围。
...
- Project 资源库外观：底部 Project 面板已切换为资源库浏览器样式，并将图库区域固定加高到约 `260px`，包含模型库、POI库、主题库、组合库、环境库、图表库、图片库七个页签，以及筛选占位行和横向资源卡片；模型库内置立方体、球体、地面、半球光、方向光、点光源六类基础资源，并支持导入模型文件夹展示项目内模型卡片。
...
- 创建基础对象与常用灯光：在模型库中点击或拖拽 `立方体`、`球体`、`地面`、`半球光`、`方向光`、`点光源` 内置资源卡片；点击卡片会在默认位置创建对象，拖拽到 Scene View 会按鼠标释放位置投射到地面平面并创建对象。
...
- 调整视野：在 Toolbar 中通过 `视野` 下拉选择 `近景`、`标准`、`远景`、`全景`，用于快速调整 Scene View 默认相机观察距离和可视范围。
