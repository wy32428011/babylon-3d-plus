...
- 创建基础灯光：支持创建 Hemispheric、Directional、Point 三类灯光实体；阴影默认使用静态烘焙模式（FPS 优先）：在 Inspector 场景属性中点击“更新阴影”，将静态设备遮挡合成到环境原色纹理，运行时不创建实时阴影生成器、不采样实时阴影贴图。环境保持原色，透明度和隐藏继续独立生效；相机移动无需重新烘焙。修改静态布局、环境资源/变换、太阳方向或阴影浓度后显示“已过期”，需重新更新；保存允许保留待烘焙场景，发布和部署导出会阻止缺失或过期结果。遥测、动画、未知脚本、手动漫游和运行时生成对象及其后代保守排除；已核对的固定货架（newshelf）、框架（frame）、围栏（fence）参数模型可参与，参数变化会使结果过期，不留下运动设备旧位置的影子。烘焙按设备位置筛选地面，普通 UV 优先合成原色；厂区重复/平铺 UV 地面保留原纹理，并共用静态遮罩（增加一次普通纹理采样，不进行实时阴影生成）。同一遮罩只保存一份 PNG，其它表面通过 textureRef 引用。上下重叠楼层自动分层烘焙，并合成一张分区遮罩贴图，无需手动拆分环境；各分区分辨率按既有纹理预算自动分配。无有效静态设备或纹理数据超限时显示“更新失败”和原因，不生成空白结果。结果以 PNG 随场景保存，原资源不覆盖；单边最多 4096 像素、合计最多 16M 像素（RGBA 基础层约 64 MiB，另有 mipmap），会增加纹理内存，需在目标业务场景实测帧耗时。显式选择实时阴影后，Directional 接管主阴影，没有可见方向光时使用自动太阳；性能/均衡档使用随模型运动刷新的缓存贴图，高质量档使用实时级联阴影，均有额外 CPU/GPU 开销。编辑态中的 Point 使用可拾取球形标记，Directional 使用沿实际照射方向显示的箭头标记；标记保持近似固定屏幕尺寸、接受正常深度遮挡，并在运行预览和发布 Viewer 中隐藏。
