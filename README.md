...
- RGV 运动箭头：在“数据驱动 → RGV 运动箭头”中配置行走、前工位和后工位三路特效。行走沿固定双轨中心正上方覆盖全长，前后台面箭头随车移动；复用输送线十种样式，支持独立预览、诊断、撤销、复制、保存和 SOURCE/DIST。默认关闭，正常停止短淡出，故障、断流或绑定冲突立即隐藏；使用方法、模型声明要求与验证入口见 [RGV 运动箭头](docs/rgv-motion-arrows.md)。

- 堆垛机运动箭头：在模型“数据驱动 → 运动箭头”中启用，行走、升降、前叉和后叉按三维运行时实际执行的运动显示方向；行走箭头覆盖固定轨道全长、不随底盘平移，升降箭头按载货台当前高度留出随平台移动的缺口；支持特效库十种箭头样式、挂点与范围校准、四路独立预览、撤销、复制、保存及 SOURCE/DIST。默认关闭，旧场景保持原貌；正常停止约 120 ms 淡出，故障、急停、断流或绑定冲突立即隐藏。使用方法、发布升级要求与验证入口见 [堆垛机运动箭头](docs/stacker-motion-arrows.md)。

- 六类输送方向箭头特效：特效库新增单一直线、连续流向、分段式、宽幅带式、双列前进和高速流动六种发光箭头及缩略图。可独立拖入场景调整长宽、颜色、强度、速度、不透明度、数量和反向，也可拖入输送线“表面箭头”样式框；支持保存、撤销、复制和 SOURCE/DIST。使用说明见 [输送方向箭头特效](docs/conveyor-arrow-effects.md)。

- 输送线表面箭头：输送线默认启用“移动双箭头”并关闭呼吸效果（已保存的显式设置保持），可将特效库的十种内置箭头拖入属性样式框，继承模型方向映射或单独指定 MQTT 点位及正反停编码。支持部件校准、范围/颜色/流速/呼吸调整和临时预览；拖放只改当前模型样式，参数支持撤销、复制及保存。停止、故障或过期时隐藏，已有 Viewer 需重新构建发布；使用方法与验证边界见 [输送线表面箭头](docs/conveyor-surface-arrows.md)。

- 可编辑组合模型：将两个及以上模型拖到组合库空白处新建；调整后拖回已有卡片可替换同一资源。组合支持整体操作、内部编辑、独立实例、历史版本与中台环境模型管理同步，并保留 SOURCE/DIST 和项目归档链路。使用方式及升级要求见 [可编辑组合模型](docs/editable-compositions.md)。

- 场景主题：主题库中的“科技蓝夜景”支持点击或拖入场景，一次应用冷蓝底光、主光、环境受光、曝光和距离雾，并可在场景属性微调、恢复默认或停用。灯光属性补充颜色、点光范围和暖白作业灯快捷设置。参数支持撤销重做、保存重开及 SOURCE/DIST，旧场景默认保持原貌；新版 Viewer 需重新构建发布。使用方式、兼容行为和验证命令见 [场景主题](docs/scene-themes.md)。

- 环境模型建筑特效：轮廓、棱线、发光、扫光、高度渐变、全息、透视和溶解生长的“绑定目标”现在可选择“环境模型”；场景属性的环境区域也支持从特效库拖入，自动创建同一种绑定。切换建筑外观类型保留环境绑定，调参无需重载 GLB；配置随普通特效实体保存、撤销和发布，清除环境后保留绑定并在环境恢复时重新应用。


- 数字孪生特效库：新增建筑材质与结构、区域扫描、三维路径、空间数据和环境及设备报警类 43 个可配置组件，并提供现有告警、图表立标、点击信息卡与镜头巡检的复用入口。旧雷达、定位光柱、火焰、烟雾从新建入口隐藏，旧场景继续兼容；输送方向箭头、移动双箭头及管线流动箭头已恢复库入口，可拖入输送线表面箭头样式框。详见 [特效组件与 44 项示意映射](docs/digital-twin-effects.md)。配置支持撤销、复制、保存重开及 SOURCE/DIST；新版运行时需重新构建并重新发布。
- 特效详细配置与数据绑定：61 种可见独立 EFF 中，52 种提供按类型登记的 367 项专用参数，恢复的三种箭头沿用通用参数，六种新输送箭头提供专用尺寸与外观面板，并支持模型库模板拖入、场景/环境目标、数据源与字符串资产编号匹配、MQTT/中台托管 HTTP、字段映射/空间数据集、条件触发和运行诊断。路径及区域可直接在场景绘制；实时值只作用于运行副本。旧特效需显式启用详细配置，旧场景仍兼容。操作与发布说明见 [特效详细配置](docs/effect-configuration-v2.md)。
- 目标跟随模型类型：允许编辑场景零实例，运行时从场景模型、生成器输出及库存薄实例中定位；支持实例来源、生成器范围、资产/容器/承载设备编号及运行时选择。目标加载后跟随，消失时保持镜头，同业务身份重建后恢复；会话选择不写入场景，类型引用随 SOURCE/DIST 的实际生产者资源一起迁移。详见 [类型跟随与数据绑定](docs/effect-configuration-v2.md)。


- 光墙围栏：从特效库添加沿建筑外围的闭合动态光墙，底部明亮、向上逐渐透明。支持颜色、高度、透明度与向上流动速度（0 为静止），默认 10 × 8 米矩形，可按长宽生成矩形或输入 3–128 个局部 X/Z 轮廓点贴合不规则建筑。轮廓首尾自动闭合，拒绝自交、重叠及无效点；用变换工具摆放，缩放会同时改变实际高度。参数与轮廓支持撤销、复制、保存重开及 SOURCE/DIST；Viewer 使用同一材质且无需额外纹理资源，已有发布需要重新发布。轮廓文本点击“应用轮廓”后才进入场景保存。

- 光墙围栏验证：`node --experimental-strip-types --test --test-concurrency=1 tests/editor/lightWallFence.test.ts tests/editor/lightWallFence.integration.test.mjs tests/runtime/lightWallFence.test.mjs`；编辑器 WebGL 回归运行 `node scripts/smoke-light-wall-fence.mjs`。完成 `npm run build` 后运行 `node node_modules/electron/cli.js tests/digitalTwin/lightWallFencePackages.integration.mjs`，再运行 `node scripts/smoke-light-wall-viewer.mjs` 验证实际 DIST。上述使用本地示例建筑/轮廓和平台配置 fixture，不代表已部署至真实业务场景。

- 发布 Viewer 版本缓存：首次加载把场景、资源清单、模型、外链纹理、脚本及天空盒文件存入 IndexedDB；同版本刷新复用文件，并复用 Draco、Meshopt 几何解码及 KTX2 纹理转码结果，HDR/受支持 EXR 沿用已有天空盒解码缓存。每次发布生成独立 `cacheRevision`，刷新先读取最新配置，新版本自动重新加载；下载期间发生发布变更会提示重新加载，避免混用版本。项目运行配置、启停、业务接口与 MQTT 仍实时读取。浏览器刷新仍需重建场景、运行脚本、解析普通 PNG/JPEG、上传 GPU 及预过滤，不能保证零初始化。缓存按部署地址、发布标识、解码参数和引擎版本隔离，KTX2 额外区分显卡能力；资源与模型/纹理解码缓存总计上限 1 GiB、4096 条，单条超过 512 MiB 不缓存，达到限额时淘汰较久未使用项。浏览器清理存储、空间不足或禁用 IndexedDB 时自动恢复正常加载。普通 HTTP 内网可用，无需 Service Worker；新版 Viewer 兼容缺少版本标识但具有完整资源哈希清单的旧格式配置，按清单指纹缓存模型和天空盒；无完整哈希时保持联网读取。已部署的旧 Viewer 需要使用新版编辑器重新发布后才能生效。验证命令和边界见 [发布 Viewer 缓存](docs/published-viewer-cache.md)。
...
- 区域视角回归：`node scripts/smoke-region-views.mjs` 验证 Store、撤销、序列化与真实 Babylon 相机控制，并生成 `output/region-views/scene.scene.json`；完成 `npm run build` 后可运行 `node node_modules/electron/cli.js tests/digitalTwin/regionViewPackages.integration.mjs` 检查双包中的实际场景条目。上述本地测试不代表已向业务项目部署。
...
- 堆垛机聚焦：编辑器、运行预览与发布 Viewer 共用梯笼端面的正向平视近景，观察方向随模型旋转，目标位于下部梯笼中段，距离上限为 8 m；固定轨道不参与机身取景，普通模型、多选及货格聚焦沿用原规则。已有线上场景需重新构建并发布 Viewer 后生效。
...
- 创建基础灯光：支持创建 Directional（方向光）、Spot（聚光灯）、Point（点光源）、Hemispheric（半球光）、RectArea（矩形面光）五类灯光实体；阴影默认使用静态烘焙模式。在 Inspector 场景属性中点击“更新阴影”，将所有可见实体模型（包括参数、脚本、遥测和动画模型）的当前姿态烘焙到环境接收面；运行时不创建实时阴影生成器。相机移动无需重新烘焙，模型移动或修改参数、环境、太阳方向、阴影浓度后需要更新；发布和导出会阻止缺失或过期结果。地面使用独立静态遮罩并保留原色和平铺纹理，上下重叠楼层自动独立烘焙，不要求人工拆分。阴影按实际投影范围分配像素，目标 128 像素/米，单边上限 8192；分块高质量 PCF 计算避免远处模型扩大深度图范围。每层贴图独立保留精度，同层共享 PNG 引用；总纹理预算为 128M 像素，PNG 数据总预算为 32 MiB。更清晰的烘焙会增加更新时间、材质变体及纹理显存（RGBA 基础层最高 512 MiB，另有 mipmap），运行时每个接收面增加一次普通纹理采样，FPS 需以目标场景和显卡实测。结果随场景保存，原资源不覆盖；运动后的实时阴影和模型之间的动态遮挡仍需实时模式。显式选择实时阴影后，Directional 接管主阴影，没有可见方向光时使用自动太阳；Point/Spot 在实时模式额外生成局部投影，Point 使用六面贴图，开销高于 Spot；Hemispheric/RectArea 不生成投影。方向光在性能/均衡档使用随模型运动刷新的缓存贴图，环境与阴影地面接收投影；高质量档使用实时级联阴影，以阴影距离限定覆盖范围，避免自动深度收紧裁掉最远的平面接收面，普通模型也接收投影，均有额外 CPU/GPU 开销。编辑态中的 Point 使用可拾取球形标记，Directional 使用沿实际照射方向显示的箭头标记，Spot 使用锥形标记，RectArea 使用矩形发光面标记；标记保持近似固定屏幕尺寸、接受正常深度遮挡，并在运行预览和发布 Viewer 中隐藏。
...
- Inspector 灯光编辑：支持编辑灯光类型、强度（允许 0）、颜色和夜间行为；Point/Spot 可设置照射范围，Spot 可设置完整光锥角度（界面使用度，保存为弧度）与光束衰减，RectArea 可设置发光面宽高（米），Hemispheric 可设置天空和地面颜色。Point 只显示 position，Directional/Spot/RectArea 显示 position 与 rotation，Hemispheric 将底层 position 字段按实际语义显示为 direction；所有灯光均隐藏无效 scale。新参数支持撤销、保存重开、运行预览和发布 Viewer，旧场景保持原有缺省值。
- Transform Gizmo：Scene View 中支持移动、旋转、缩放三种可视化操控模式，普通拖拽结束后写入撤销/重做历史；Point 灯光只开放移动工具，Directional/Spot/RectArea 开放移动与旋转工具，选择无效工具会自动回退移动，Hemispheric 不显示场景 Gizmo。单文件夹和任意多选临时强制使用世界坐标，只开放移动与旋转，不支持缩放或 Shift 阵列；退出群组选区后恢复进入群组前的工具与局部/世界坐标偏好。编辑模式下选中单个未锁定可阵列实体并使用移动工具时，可按住 `Shift` 拖动 X/Y/Z 单轴箭头进入模型阵列。可阵列实体包括导入模型、内置 Mesh、虚拟定位线框、已解锁 CAD 参考层和特效；文件夹、灯光、全局唯一模型生成器和球形天空盒明确排除阵列，灯光按住 `Shift` 时继续执行普通移动。自动巡检路线只开放移动与旋转，节点子目标同样禁止缩放并关闭 Z 轴滚转；节点拖动只回写对应巡检点位，不改变路线实体 Transform。
...
- JSON 场景保存/加载：支持将当前场景保存为 JSON 文件，并从 JSON 场景文件加载；保存、文件选择加载和首页最近场景加载成功后都会更新最近场景列表。
 - Project 资源库外观：底部 Project 面板已切换为资源库浏览器样式，位于中间列 Scene 画布下方且与 Scene 同宽，并将图库区域加高到约 `300px` 至 `460px` 自适应，包含模型库、POI库、特效库、主题库、组合库、环境库、天空盒库、图表库、图片库九个页签，以及筛选占位行和可换行资源卡片；模型库卡片使用深色直角卡、上方缩略图、下方两行居中文字和单行省略标题，模型库内置立方体、球体、地面、虚拟定位线框、方向光、聚光灯、点光源、半球光、矩形面光等基础资源，以及环境光 HDRI/IBL 入口；六类光源卡片采用独立照明示意缩略图并说明发光方式和阴影条件；“环境光 HDRI/IBL”卡片打开天空盒库以导入或选择 HDR/EXR，复用全局环境贴图提供照明和反射，不创建重复灯光实体。IBL 不自动投影，当前未接入 IBL Shadows。HDR/EXR 天空盒资源展示在天空盒库，点击天空盒卡片或拖入 Scene 会创建/更新一个进入 Hierarchy 的可见球体并选中，重复放置只更新这个全局唯一实体。图表库会在当前工程绑定数据中台项目后，自动同步该项目绑定的完整大屏卡片，并支持手动重新同步；不解析或展示大屏内部图表，未绑定本地项目不访问数据中台，也不会展示其它项目的大屏；同步仅保存大屏索引元数据，不保存大屏 `jsonContent`，大屏卡片当前仅用于展示，不能点击创建或拖入 Scene View/相机视窗。POI 库保留可点击或拖入 Scene 任意位置的“模型生成器”，重复创建入口会选中已有生成器而不是新建副本；同时新增可创建多条路线的“自动巡检”卡片，点击或拖入 Scene 后进入 Hierarchy；特效库展示新版数字孪生 EFF 和既有组件复用入口，支持点击与拖拽创建，并兼容旧场景中的 EFF；环境库使用独立的单 GLB 文件导入入口，支持点击应用或拖入右侧“环境模型”整条属性行；天空盒库支持导入 HDR/EXR，也可拖入右侧“天空盒资源”区域；所有导入模型进入场景后统一以米为操作单位。
...
- 切换 Gizmo：点击顶部工具栏的移动、旋转、缩放图标按钮，或使用 W/E/R 快捷键。点光源只允许移动，方向光、聚光灯和矩形面光允许移动和旋转；灯光请求不支持的工具时会自动切回移动。
...
- 编辑属性：在 Inspector 中修改名称、Transform、材质颜色或灯光属性；position 按米输入，rotation 按角度输入但内部仍使用 Babylon 弧度，通用 scale 保持无量纲。灯光字段按有效语义收敛：Point 仅 position，Directional/Spot/RectArea 为 position/rotation，Hemispheric 为 direction。内置 Box 的 `size (m)` 直接对应实际边长，Sphere/Plane 显示其米制基准说明；导入模型、环境模型和 CAD 均展示源单位到米的换算信息。
...
- 灯光支持五种类型及各自有效参数，模型库另提供 HDRI/IBL 环境光入口。点光源和聚光灯投影需要开启场景实时阴影；矩形面光与半球光不投影。环境模型默认保留原色，与光源隔离；需要环境模型参与灯光照明时，在主题配置中启用“环境受光”。场景级阴影可在未选中对象时的 Inspector 场景属性中调节。
...
- 2026-09-20：货架库存 fetch 同步新增定时全量触发：工具栏“配置 Fetch 请求”可设同步间隔（秒，默认 60、最小 10，填 0 表示只在进入运行预览时同步一次），编辑器运行预览与数字孪生发布包 Viewer 都按该间隔持续拉取，覆盖外部改库存的盲区。定时 tick 遇到在途请求（如放货单排同步）会跳过本轮，避免抢走响应代际导致格口抑制不解除、停留货箱不销毁；定时失败提示只打印一次，链路恢复后再失败会重新提示。间隔为场景配置项，随场景保存并进入发布包。
- 2026-09-20：货架格口抑制新增数据确认解除：设备取货后该格被抑制隐藏货物，此前要等同一排下次放货才恢复可见，期间外部入库的新货不显示；现在任何 fetch 响应（含定时全量）确认该格已空即自动恢复该格渲染。该排仍有待清算保留货箱时不做解除，避免与保留货箱双显；取货本身仍不触发单排同步（服务端此刻常未更新，会双显）。
...
- 面板形状支持矩形和 360° 环形。环形是上下镂空的曲面屏，可设置半径（默认 3 m）、内容平铺次数（默认 4；设为 1 时整圈显示一份连续画面）、展开内容尺寸、悬浮高度和青色边框。环形固定跟随实体变换，不启用面向摄像机；切回矩形保留原有设置。旧场景缺省仍为矩形，支持撤销、复制、保存重开和 SOURCE/DIST。
- 环形文字、背景图片及允许 CORS 的视频共用连续曲面纹理；运行时双击环形大屏或视频打开原始内容交互，关闭后保持页面状态。大屏画面每约 1.2 秒按需采集一次，每立标仅一个 iframe，整圈共用最多 2 MP 的纹理；隐藏或删除时释放页面及纹理。
- 实时网页曲面需要同时更新数据中台 Preview/Published 截帧桥，并使用与中台同源的 HTTP(S) Viewer；开发模式另支持本机跨端口。Electron file://、生产跨域或未更新的中台不会获准回传画面，会显示提示，仍可在运行态双击打开原始页面。大屏内的视频、嵌套 iframe、不可读画布用明确占位显示；直接视频模式需要媒体服务器允许跨域访问。
...
报警参考图中的 12 类外观可在“参考报警样式”直接选择：设备变色、闪烁、呼吸光晕、描边、顶部灯、悬浮图标、光柱、波纹、地面警戒圈、信息卡、烟雾粒子和故障路径。特效库新增其中缺失的 6 种独立 EFF；火焰与火花继续从特效库选择。局部外观预设保留设备本色，需要同时染色时勾选“覆盖设备颜色”；旧场景缺省保持原有颜色覆盖。操作、参数和验证见 [12 类报警外观](docs/alarm-reference-effects.md)。
...
3. `CUSTOM PROPERTY` 监听绑定设备的 MQTT 点位：火警属性填写 `data` 中的 `p` 名称，触发值填写该点位希望命中的 `v`。例如 `data:[{p:"fireAlarm",v:1}]` 对应火警属性 `fireAlarm`、触发值 `1`，无需填写数组下标；实时值不会写回触发值。默认仍为 `fireAlarm` / `true`，同名属性优先精确匹配，保留旧点路径兼容；值按去除首尾空格、忽略大小写的标量文本比较。触发值 `true/false` 兼容相应的布尔值和 `1/0`，触发值 `1/0` 仅匹配对应数值或字符串，保留旧场景的单向兼容。启用“仓库告警”后，还监听 `warehouseAlarm=true/1` 或配置的火警条件（CUSTOM PROPERTY 保留最后值，RUNNING STATE 沿用超时判断），优先展示仓库告警主题；只需普通火警时关闭该开关。
4. “外观特效”从特效库拖入 EFF 卡片，不能拖入模型库资源；设备目标槽仍从模型库选取。报警时为每台命中的绑定设备独立显示特效，随设备世界位置和旋转更新：火焰、烟雾、警示灯等位于设备顶部，光圈、光柱、围栏等以设备底部中心为锚点，使用可编辑的米制尺寸。拖入后展开“报警外观属性”，可修改颜色、强度、速度及对应类型的尺寸、透明度和专用参数；这些值仅写入当前报警管理器的外观配置，支持撤销、保存重开和发布。目标绑定及触发仍由报警管理器控制，无需为外观再次配置数据源。轮廓、高亮、全息等模型表面特效直接作用于绑定模型，优先显示自身外观，暂停该设备的普通颜色覆盖和原有模型特效，解除后恢复；其余特效仍叠加报警覆盖颜色。外观为空时显示内置火焰；旧场景已保存的模型外观继续兼容，拖入特效或清空后替换旧外观。场景雾、昼夜切换、镜头跟随及报警/立标/巡检等业务入口不能作为设备局部外观。“摄像机聚焦”开启时，新报警触发一次设备聚焦。报警解除、隐藏目标、停止预览或删除管理器后，清除派生效果并恢复原材质。共享模型中的其它设备不会被染色；受监控设备及其原有批次伙伴会独立渲染，大批量监控时需留意绘制调用增加。
...
7. 配置支持撤销重做、复制、保存重开；同批复制的具体设备引用会重映射。发布包和源工程包保存外观特效配置，并收集目标模型、旧版外观模型及关联资源，保留主题引用与内嵌背景图片，不依赖编辑电脑的绝对模型路径。旧场景无报警组件时保持原行为。

MQTT 点位诊断按目标设备分别显示资产编号、设备类型、数据源、当前 `v`、触发值、最后接收时间及未触发原因。编辑态可手动配置；运行预览时展开诊断，每 500 ms 更新当前页（每页 10 台设备），收起或离开运行预览后停止刷新。诊断不会产生场景修改或撤销记录，运行预览中仍可翻页。零设备时分别提示“未配置报警目标”或“未匹配到场景设备”，此时尚未进入点位判断。

模型目标使用中台来源、资源 ID 与包内模型路径匹配，同一模型同步后缓存位置变化仍可监控。普通模型库条目或旧场景缺少身份时，兼容模型包的资源类别、资源 ID 与完整包内路径，因此共享库模型可匹配场景中的固定版本实例；普通本地模型仍按精确资源路径匹配，兼容 URL 编码与修订参数差异，不按名称或文件名猜测。已知来源冲突时不会匹配；来源缺失且同一资源出现多个来源时，仅保留精确路径匹配，或由用户在 `ENTITY` 中选择具体设备。带来源标识的库模型导入场景时会保留稳定身份。配置 `CUSTOM PROPERTY`、火警属性 `normal`、触发值 `true` 后，绑定设备消息 `data:[{p:"normal",v:true}]` 会按自定义条件报警，使用覆盖颜色和配置的外观特效；该模式不受内置 `normal=true` 表示正常的运行状态解释影响。`normal=false` 不匹配该条件，会清除效果（其他独立报警条件仍按各自配置判断）。

设备身份沿用 `sourceId + deviceType + assetCode`；同名 `p` 不跨设备或数据源匹配。原始 EPV 遥测仍按整帧替换；CUSTOM PROPERTY 单独按设备和订阅点位保留本次运行最后一次收到的值。没有新消息、仅更新其它点位或该点位没有 `v` 时均保留当前状态，不因设备超时解除；相同值持续报警且不重复激活，收到不匹配的新值或显式无效值时解除。停止预览、删除管理器或显式清空数据后释放保留状态。普通文档同步保留未变订阅，RUNNING STATE 的离线超时规则保持。报警逐条接收点位更新，并约每 250 ms 评估一次，短于评估间隔且已被后续值覆盖的瞬时脉冲不保证显示。JSON Path 适配器继续使用映射后的字段名；诊断基于共用标准化快照，并非原始点位目录。

验证：`node --test --test-concurrency=1 tests/editor/alarmTargetDrop.test.mjs tests/editor/alarmCustomProperty.test.mjs tests/editor/alarmManager.test.mjs`；`node scripts/smoke-alarm-custom-property.mjs` 验证真实浏览器表单、只读诊断、最后值持续保持、新值解除和分页；`npm run build:electron` 后执行 `node_modules/.bin/electron.cmd tests/digitalTwin/alarmManagerPublish.integration.mjs` 验证部署资源与源工程包；`node scripts/smoke-alarm-manager.mjs` 在 Edge 中验证真实 WebGL 颜色隔离、火焰、立标可见像素、解除恢复和 GLB 外观加载，截图位于 `output/playwright/alarm-manager/`。`node scripts/smoke-alarm-appearance.mjs` 验证完整编辑器的特效库拖放、对应属性编辑、拒绝模型、保存重开、实际尺寸与颜色像素、多设备隔离和停止清理，截图位于 `output/playwright/alarm-appearance/`。新版报警特效需构建并重新发布 Viewer 后才能用于已有线上场景。真实现场 MQTT 与数据中台在线大屏需使用项目实际连接配置联调。
