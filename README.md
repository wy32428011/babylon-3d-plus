...
- 输送线表面箭头：输送线默认开启呼吸箭头（显式关闭保持），可将特效库的十种内置箭头拖入属性样式框，继承模型方向映射或单独指定 MQTT 点位及正反停编码。支持部件校准、范围/颜色/流速/呼吸调整和临时预览；拖放只改当前模型样式，参数支持撤销、复制及保存。停止、故障或过期时隐藏，已有 Viewer 需重新构建发布；使用方法与验证边界见 [输送线表面箭头](docs/conveyor-surface-arrows.md)。
