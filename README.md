...
- 输送线表面箭头：输送线实例可配置贴合输送面的流动箭头，继承 MQTT 设备绑定及正反向映射，停止、故障或数据过期时隐藏。支持表面部件校准、范围/外观调整和不入场景的临时预览，参数沿用撤销重做、复制与场景保存。旧场景默认关闭，已发布 Viewer 需重新构建发布；使用方法与验证边界见 [输送线表面箭头](docs/conveyor-surface-arrows.md)。
...
- 数字孪生特效库：新增建筑材质与结构、区域扫描、三维路径、空间数据和环境类 37 个可配置组件，并提供现有告警、图表立标、点击信息卡与镜头巡检的复用入口。旧雷达、定位光柱、火焰、烟雾和流动箭头从新建入口隐藏，旧场景继续兼容。详见 [特效组件与 44 项示意映射](docs/digital-twin-effects.md)。配置支持撤销、复制、保存重开及 SOURCE/DIST；新版运行时需重新构建并重新发布。
- 特效详细配置与数据绑定：46 种可见独立 EFF 提供按类型登记的 330 项专用参数，并支持模型库模板拖入、场景/环境目标、数据源与字符串资产编号匹配、MQTT/中台托管 HTTP、字段映射/空间数据集、条件触发和运行诊断。路径及区域可直接在场景绘制；实时值只作用于运行副本。旧特效需显式启用详细配置，旧场景仍兼容。操作与发布说明见 [特效详细配置](docs/effect-configuration-v2.md)。
