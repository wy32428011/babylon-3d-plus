...
export function startDataPlatformModelSync(baseUrl: string, editorRoot: string): boolean {
  ...
  if (activeModelSync) return true;
  ...
  const promise = runDataPlatformModelSync(runId, context, controller.signal)
    .catch((error: unknown) => {
      if (controller.signal.aborted && modelSyncShuttingDown) return;
...
async function runDataPlatformModelSync(
  ...
  try {
    ...
    const finalIndex: ProjectAssetIndex = { version: 2, assets: finalAssets };
...
function normalizePagedResponse(value: unknown, label: string): { records: unknown[]; total: number } {
...
function normalizeModelScripts(record: Record<string, unknown>, index: number): Array<{ fileName: string | null; fileUrl: string }> {
  const scripts: Array<{ fileName: string | null; fileUrl: string }> = [];
  const seenUrls = new Set<string>();

  const append = (fileName: unknown, fileUrl: unknown) => {
...
async function finalizeThumbnailPath(destinationPath: string, contentType: string, remoteUrl: string): Promise<string> {
...
async function validatePreparedPackages(
  ...
  for (const prepared of packages) {
    if (signal.aborted) throw new Error('数据中台任务已取消。');
...
async function validateModelFile(filePath: string): Promise<void> {
  ...
  if (extension === '.glb') {
    if (!(await validateGlbModelFile(filePath))) throw new Error(`GLB 文件结构无效或已损坏：${path.basename(filePath)}`);
    return;
  }
...
function relocateAssetEntry(asset: ProjectModelAssetEntry, stagingRoot: string, editorRoot: string): ProjectModelAssetEntry {
  ...
  const finalScriptPaths = asset.scriptPaths?.map((item) => relocate(item)).filter((item): item is string => Boolean(item));
  ...
  try {
    ...
    if (rollbackErrors.length > 0) {
      throw new DataPlatformRollbackError(
        `${message}；模型库回滚不完整：${rollbackErrors.join('；')}；已保留恢复目录：${backupRoot}`,
      );
    }
    throw error;
  }
