...
export async function activateProjectRoot(
  projectRoot: string,
  lastScenePath?: string,
): Promise<ProjectListAssetsResult> {
  const normalizedProjectRoot = normalizeFilePath(projectRoot);
 ...
 */
async function prepareEnvironmentPackageWithGlb(
 ...
 */
export async function importEnvironmentModelFileIntoProject(
  sourceFilePath: string,
): Promise<{ importedAsset: ProjectModelAssetEntry; projectAssets: ProjectModelAssetEntry[] }> {
  const projectRoot = await loadRecentProjectRoot();
  ...
  try {
    ...
    const projectAssets = [
      ...currentIndex.assets.filter(
        (asset) => asset.libraryKind !== 'environment'
