...
import {
  assertSafeDirectory,
  copyDeploymentFiles,
  createDeploymentExportAbortError,
  isDeploymentExportAbortError,
  isPathInsideOrEqual,
  lstatIfExists,
  ...
  type DeploymentCopyProgress,
} from './deploymentExportFileSystem.js';
import {
  createAssetManifestContent,
  prepareDeploymentExport,
} from './deploymentExportScene.js';
...
type ActiveDeploymentExport = {
  requestId: string;
  sender: WebContents;
  controller: AbortController;
  stagingPath: string | null;
  temporaryZipPath: string | null;
};

type CompletedDeploymentExport = {
  requestId: string;
  outputPath: string;
...
type ValidatedSender = {
  sender: WebContents;
  ownerWindow: BrowserWindow;
};

const activeExports = new Map<number, ActiveDeploymentExport>();
const completedExports = new Map<number, Map<string, CompletedDeploymentExport>>();
const cleanupBoundSenderIds = new Set<number>();

/** 注册 Web 部署工程导出、取消、进度和定位 IPC。 */
export function registerDeploymentExportIpc(): void {
...
export function disposeAllDeploymentExportTasks(): void {
  for (const task of activeExports.values()) task.controller.abort();
  activeExports.clear();
...
async function handleStartExport(
  ...
  request: DeploymentExportRequest,
  ...
  try {
    ...
    if (activeExports.get(sender.id) === task) activeExports.delete(sender.id);
  }
}

/** 取消当前 sender 中 requestId 对应的活动任务。 */
function handleCancelExport(event: IpcMainInvokeEvent, request: DeploymentExportCancelRequest): boolean {
  ...
  if (!task || task.requestId !== requestId) return false;
  task.controller.abort();
  return true;
}

/** 只允许当前 sender 按已完成 requestId 定位正式发布结果。 */
async function handleRevealExport(event: IpcMainInvokeEvent, request: DeploymentExportRevealRequest): Promise<void> {
  const { sender } = assertTrustedSender(event);
  const requestId = validateRequestIdRequest(request, '定位导出结果');
  const completed = completedExports.get(sender.id)?.get(requestId);
  if (!completed) throw new Error('只能定位当前窗口中已经完成的导出 requestId。');

  const stat = await lstatIfExists(completed.outputPath);
  if (!stat) throw new Error('导出结果已被移动或删除。');
  if (stat.isSymbolicLink()) throw new Error('导出结果已被替换为符号链接或 Junction，拒绝定位。');
...
async function runDeploymentExport(
  ...
  try {
    ...
    if (!destination) return createCanceledResult(request);
    ...
    if (destination.temporaryZipPath) forbiddenOutputPaths.push(destination.temporaryZipPath);
    ...
    const prepared = await prepareDeploymentExport(
      ...
      signal,
      (message) => emitProgress('preflight', message, 8),
    );
    ...
    if (request.format === 'zip') {
      if (!destination.temporaryZipPath) throw new Error('ZIP 临时文件路径缺失。');
      emitProgress('archive', '正在生成 ZIP 压缩包…', 84, totalFiles, totalFiles, totalBytes, totalBytes);
      await archiveDeploymentDirectory(
        destination.stagingPath,
        destination.temporaryZipPath,
        path.basename(destination.finalPath, path.extname(destination.finalPath)),
    ...
    if (isDeploymentExportAbortError(error) || signal.aborted) return createCanceledResult(request);
    throw error;
  } finally {
...
function assertTrustedSender(event: IpcMainInvokeEvent): ValidatedSender {
  const sender = event.sender;
  const ownerWindow = BrowserWindow.fromWebContents(sender);
  if (!ownerWindow || ownerWindow.isDestroyed() || sender.isDestroyed()) throw new Error('导出请求 sender 无效。');
  if (!event.senderFrame || event.senderFrame !== sender.mainFrame) throw new Error('导出请求只能由主 frame 发起。');
  if (!isAllowedRendererUrl(sender.getURL())) throw new Error('导出请求来自未授权的 renderer URL。');
...
function validateDeploymentExportRequest(request: DeploymentExportRequest): DeploymentExportRequest {
  if (!isPlainObject(request)) throw new Error('Web 部署工程导出请求格式不正确。');
  const requestId = validateRequestId(request.requestId);
  if (typeof request.suggestedName !== 'string') throw new Error('导出工程名称格式不正确。');
  const name = request.suggestedName.trim();
  if (!name || name.length > 128 || /[\u0000-\u001f]/.test(name)) throw new Error('导出工程名称必须是 1 到 128 个有效字符。');
  if (request.format !== 'directory' && request.format !== 'zip') throw new Error('导出格式只支持 directory 或 zip。');
  if (typeof request.sceneContent !== 'string' || !request.sceneContent) throw new Error('导出场景内容不能为空。');
...
function validateRequestIdRequest(request: unknown, label: string): string {
  if (!isPlainObject(request)) throw new Error(`${label}请求格式不正确。`);
...
async function selectExportDestination(
  ownerWindow: BrowserWindow,
  request: DeploymentExportRequest,
  signal: AbortSignal,
  ...
  if (request.format === 'directory') {
    ...
    if (result.canceled || !selectedParent) return null;
  ...
  if (targetStat?.isSymbolicLink()) throw new Error('ZIP 目标不能是符号链接或 Junction。');
  if (targetStat?.isDirectory()) throw new Error('ZIP 目标不能是目录。');
  return createDestinationPaths(parentPath, finalPath, true);
}

/** 为正式目标生成同父目录且带随机标记的 staging 与 ZIP 临时文件路径。 */
function createDestinationPaths(parentPath: string, finalPath: string, includeTemporaryZip: boolean): ExportDestination {
...
async function createUniqueDirectoryPath(parentPath: string, baseName: string): Promise<string> {
...
function createSafeExportFileName(name: string): string {
  let safeName = name
    .normalize('NFKC')
    .replace(/[<>:"/\\|?*\u0000-\u001f]/g, '_')
    .trim()
...
function assertNoCopyPlanCollisions(templateFiles: DeploymentCopyFile[], assetFiles: DeploymentCopyFile[]): void {
  ...
  for (const file of [...templateFiles, ...assetFiles]) {
    ...
    if (destinations.has(key)) throw new Error(`导出文件目标冲突：${file.destinationRelativePath}`);
    destinations.add(key);
  }
}

/** 写入主进程生成的 runtime、scene、manifest 与部署 README。 */
async function writeGeneratedDeploymentFiles(
  stagingPath: string,
...
async function archiveDeploymentDirectory(
  stagingPath: string,
...
async function publishZipFile(temporaryZipPath: string, finalPath: string, warnings: string[]): Promise<void> {
  const targetStat = await lstatIfExists(finalPath);
  if (!targetStat) {
    await fs.rename(temporaryZipPath, finalPath);
    return;
  }
  if (targetStat.isSymbolicLink() || !targetStat.isFile()) throw new Error('ZIP 正式目标不是安全的普通文件。');
...
async function cleanupOwnedPath(targetPath: string, expectedParent: string, marker: string): Promise<void> {
  ...
  if (!stat) return;
  if (stat.isSymbolicLink()) throw new Error('拒绝清理被替换为符号链接或 Junction 的任务路径。');
  await fs.rm(normalizedTarget, { recursive: stat.isDirectory(), force: true });
}

/** 保存有限数量的已完成结果，供 reveal API 按 requestId 查询。 */
function rememberCompletedExport(sender: WebContents, completed: CompletedDeploymentExport): void {
  let records = completedExports.get(sender.id);
  if (!records) {
    records = new Map<string, CompletedDeploymentExport>();
  ...
  while (records.size > MAX_COMPLETED_EXPORTS_PER_RENDERER) {
    const oldestRequestId = records.keys().next().value as string | undefined;
...
function createCanceledResult(request: DeploymentExportRequest): DeploymentExportResult {
  return {
    requestId: request.requestId,
    canceled: true,
    format: request.format,
...
function createProgressEmitter(task: ActiveDeploymentExport): (
  ...
  return (
    ...
    if (task.sender.isDestroyed() || activeExports.get(task.sender.id) !== task) return;
    const progress: DeploymentExportProgress = {
...
function emitCopyProgress(
  emitProgress: ReturnType<typeof createProgressEmitter>,
  phase: DeploymentExportPhase,
  message: string,
  progress: DeploymentCopyProgress,
  startPercent: number,
  endPercent: number,
  baseFiles: number,
  baseBytes: number,
  ...
  emitProgress(
    phase,
    message,
    startPercent + (endPercent - startPercent) * Math.max(0, Math.min(1, ratio)),
    baseFiles + progress.completedFiles,
    totalFiles,
    baseBytes + progress.completedBytes,
    totalBytes,
  );
}

/** 安全累加复制计划中的文件字节数。 */
function sumFileBytes(files: Array<Pick<DeploymentCopyFile, 'size'>>): number {
  let total = 0;
  for (const file of files) {
    total += file.size;
    if (!Number.isSafeInteger(total)) throw new Error('导出文件总大小超过安全范围。');
  }
  return total;
}

/** 计算部署目录未压缩字节数，包含 Viewer、场景资源和生成的 JSON/README。 */
function calculatePublishedByteCount(
  templateFiles: DeploymentCopyFile[],
  prepared: Awaited<ReturnType<typeof prepareDeploymentExport>>,
  copiedAssets: DeploymentCopiedFile[],
): number {
