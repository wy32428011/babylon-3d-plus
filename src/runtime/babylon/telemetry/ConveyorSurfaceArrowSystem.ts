...
export class ConveyorSurfaceArrowSystem {
  ...
  tick(models: Iterable<ConveyorSurfaceArrowModel>, running: boolean, deltaSeconds: number, now = Date.now()): void {
    const entries = [...models].map(entry => ({ ...entry, binding: entry.deviceType === 'conveyor'
    ...
    const conflicts = collectSpecializedTelemetryConflictKeys(entries.flatMap(entry => entry.binding ? [entry.binding] : []));
    const retained = new Set<string>();
    const diagnostics = new Set<string>();
    for (const { entityId, model, binding, visible } of entries) {
      const config = model.telemetryBinding?.surfaceArrows;
      if (!config) continue;
      ...
      if (running) {
        const state = resolveConveyorSurfaceArrowState({ binding,
          ...
          config: travel, trajectoryDirection: model.telemetryBinding?.trajectoryDirection,
          now, conflict: binding ? conflicts.has(binding.key) : false });
        direction = state.direction;
        message = state.message;
      } else {
        const preview = conveyorSurfaceArrowSession.getPreview(entityId);
        direction = ((preview ?? 0) * resolveConveyorTrajectoryForwardSign(model.telemetryBinding?.trajectoryDirection, travel.axis)) as 1 | -1 | 0;
        message = preview === null ? '编辑状态：可预览正向、反向和停止'
          : `编辑预览：${preview === 0 ? '停止，箭头隐藏' : preview > 0 ? '正向' : '反向'}（模拟状态）`;
      }
