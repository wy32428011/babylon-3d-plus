...
const clientsByWebContentsId = new Map();
let registered = false;
/** 注册受控 MQTT IPC 通道；多次调用只会注册一次，避免热重载或测试重复绑定。 */
export function registerMqttIpc() {
    if (registered)
        return;
    registered = true;
...
export function disposeAllMqttIpcClients() {
    void Promise.allSettled(Array.from(clientsByWebContentsId.values(), (rendererClient) => disconnectRendererClient(rendererClient, '应用退出，MQTT 连接已关闭。')));
    clientsByWebContentsId.clear();
}
/** 处理 renderer 配置请求，仅接受地址和订阅等非敏感字段。 */
async function handleConfigure(event, request) {
    ...
    if (rendererClient.generation !== generation)
...
function getRendererClient(webContents) {
...
function connectRendererClient(rendererClient, generation, address, subscriptionSnapshot) {
    ...
    client.on('connect', () => {
        if (!isCurrentClient(rendererClient, generation, client))
            return;
        ...
        const completeSubscription = (subscription, error) => {
            if (!isCurrentClient(rendererClient, generation, client) || currentCycle !== subscriptionCycle)
                return;
            completedCount += 1;
            if (error) {
            ...
            if (completedCount < subscriptionSnapshot.length)
                return;
        ...
        for (const subscription of subscriptionSnapshot) {
            const options = { qos: subscription.qos };
            let completed = false;
            const completeOnce = (error) => {
                if (completed)
                    return;
                completed = true;
...
function isCurrentClient(rendererClient, generation, client) {
    return rendererClient.generation === generation && rendererClient.client === client;
}
/** 断开当前 client；失败时返回结构化结果，避免 IPC Promise 裸抛。 */
async function disconnectRendererClient(rendererClient, logMessage) {
    if (!rendererClient.client)
        return { ok: true };
...
function isSafeMqttAddress(address) {
    try {
...
function resolveSubscriptionForTopic(topic, subscriptions) {
...
function mqttTopicMatches(filter, topic) {
    ...
    for (let index = 0; index < filterLevels.length; index += 1) {
        ...
        if (filterLevel !== topicLevels[index])
            return false;
    }
