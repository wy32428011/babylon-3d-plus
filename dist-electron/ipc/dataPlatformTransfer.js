...
export function resolveDataPlatformRemoteUrl(baseUrl, value) {
...
export async function requestDataPlatformJson(options) {
    ...
    try {
        ...
        if (timedOut)
            throw new Error(`${options.context}超时，请稍后重试。`);
        if (options.signal.aborted)
            throw createCanceledError();
        if (error instanceof Error && error.message.startsWith(options.context))
            throw error;
...
export async function extractZipSecurely(archivePath, destinationRoot, signal) {
    if (signal.aborted)
        throw createCanceledError();
    const archiveStat = await fs.stat(archivePath);
    if (!archiveStat.isFile())
        throw new Error('下载的工程包不是文件。');
...
function isZipSymbolicLink(externalFileAttributes) {
    const unixMode = (externalFileAttributes >>> 16) & 0xffff;
    return (unixMode & 0o170000) === 0o120000;
}
async function readResponseTextWithLimit(response, maxBytes, context) {
    if (!response.body)
        return '';
    const chunks = [];
    let total = 0;
    for await (const rawChunk of response.body) {
        ...
        if (total > maxBytes)
            throw new Error(`${context}响应过大，已停止读取。`);
        chunks.push(chunk);
    }
