...
export function createDeploymentExportAbortError() {
    const error = new Error('导出已取消。');
    error.name = 'AbortError';
    return error;
}
/** 判断未知异常是否代表主动取消。 */
export function isDeploymentExportAbortError(error) {
    return error instanceof Error && error.name === 'AbortError';
}
/** 在长耗时步骤之间检查取消信号。 */
export function throwIfDeploymentExportAborted(signal) {
    if (signal.aborted)
        throw createDeploymentExportAbortError();
}
/** Windows 下按不区分大小写的方式生成本地路径比较键。 */
export function toLocalPathKey(filePath) {
...
/** 将部署相对路径统一转换为正斜杠格式。 */
export function toDeploymentPath(relativePath) {
    return relativePath.split(path.sep).join('/');
}
/** 将受控相对路径解析到 staging 内，并拒绝路径逃逸。 */
export function resolveDeploymentDestination(stagingRoot, relativePath) {
...
export async function assertSafeDirectory(directoryPath, label) {
    const normalizedPath = path.resolve(directoryPath);
    const stat = await lstatIfExists(normalizedPath);
    if (!stat)
        throw new Error(`${label}不存在。`);
    if (stat.isSymbolicLink())
        throw new Error(`${label}不能是符号链接或 Junction。`);
    if (!stat.isDirectory())
        throw new Error(`${label}不是目录。`);
    return fs.realpath(normalizedPath);
}
/** 校验源根目录与输出路径不重叠，避免递归复制 staging 或正式结果。 */
export function assertNoSourceOutputOverlap(sourceRoot, forbiddenOutputPaths) {
 ...
 */
export async function scanSafeSourceRoot(sourceRoot, includeRelativePaths, forbiddenOutputPaths, signal, maxFiles = DEFAULT_MAX_SOURCE_FILES) {
...
async function scanCompleteDirectory(normalizedRoot, realRoot, signal, maxFiles) {
    const files = [];
    ...
    while (pendingDirectories.length > 0) {
        ...
        if (current.depth > 128)
            throw new Error('资源目录层级过深，已停止导出。');
        ...
        for (const entry of entries) {
            ...
            if (files.length > maxFiles)
                throw new Error(`资源文件数量超过安全上限 ${maxFiles}。`);
        }
    }
    return files;
}
/** 只枚举调用方明确列出的文件，并验证从根目录到文件的每一级路径都不是链接。 */
async function scanExplicitFiles(normalizedRoot, realRoot, includeRelativePaths, signal, maxFiles) {
    if (includeRelativePaths.size > maxFiles)
        throw new Error(`资源文件数量超过安全上限 ${maxFiles}。`);
    const files = [];
    ...
    for (const rawRelativePath of includeRelativePaths) {
        ...
        if (!stat)
            throw new Error(`资源文件缺失：${toDeploymentPath(relativePath)}`);
        if (stat.isSymbolicLink())
            throw new Error(`资源文件不能是符号链接或 Junction：${toDeploymentPath(relativePath)}`);
        if (!stat.isFile())
            throw new Error(`资源引用不是普通文件：${toDeploymentPath(relativePath)}`);
        const realPath = await fs.realpath(absolutePath);
        if (!isPathInsideOrEqual(realRoot, realPath)) {
...
async function assertPathChainHasNoLinks(rootPath, relativePath) {
...
/** 构造只包含复制校验所需字段的源文件快照。 */
function createSafeSourceFile(sourcePath, relativePath, size, mtimeMs) {
    if (!Number.isSafeInteger(size) || size < 0)
        throw new Error('资源文件大小超出安全范围。');
    return {
        sourcePath,
...
export async function copyDeploymentFiles(files, stagingRoot, concurrency, signal, onProgress) {
    ...
    const copiedFiles = new Array(files.length);
    ...
    const abortWorkersFromParent = () => workerController.abort();
    if (signal.aborted)
        workerController.abort();
    else
        signal.addEventListener('abort', abortWorkersFromParent, { once: true });
    const workerSignal = workerController.signal;
    /** 汇总并限频发送复制进度，避免大文件产生过量 IPC 消息。 */
    const reportProgress = (force = false) => {
        const now = Date.now();
        if (!force && now - lastProgressAt < COPY_PROGRESS_INTERVAL_MS)
            return;
        lastProgressAt = now;
...
async function copySingleFileWithHash(file, destinationPath, signal, onChunk) {
    throwIfDeploymentExportAborted(signal);
...
function addSafeByteCount(left, right) {
    const result = left + right;
    if (!Number.isSafeInteger(result))
        throw new Error('导出资源总大小超过安全范围。');
    return result;
}
