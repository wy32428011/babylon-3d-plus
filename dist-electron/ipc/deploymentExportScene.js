...
import { assertSafeDirectory, isPathInsideOrEqual, lstatIfExists, scanSafeSourceRoot, throwIfDeploymentExportAborted, toDeploymentPath, toLocalPathKey, } from './deploymentExportFileSystem.js';
...
export async function prepareDeploymentExport(content, exportName, forbiddenOutputPaths, signal, onStatus) {
    ...
    onStatus('正在安全扫描资源包…');
    const { assetFiles, sourceUrlMap } = await createAssetCopyPlan([...bundles.values()], forbiddenOutputPaths, signal, onStatus, warnings);
...
export function createAssetManifestContent(copiedFiles) {
    const assets = copiedFiles
        .filter((file) => typeof file.logicalUrl === 'string')
        .map((file) => ({
...
function createManifestAssetPath(destinationRelativePath) {
...
function parseSceneFileV1(content) {
...
function collectModelGeneratorReferences(generator, models, entityId) {
    collectModelGeneratorTarget(generator.defaultTarget, generator, models, `实体 ${entityId} 默认生成目标`);
    if (generator.rules === undefined)
        return;
...
function collectModelGeneratorTarget(value, packageOwner, models, label) {
...
async function loadProjectAssetContext(signal, warnings) {
    throwIfDeploymentExportAborted(signal);
...
async function resolveModelReference(reference, projectContext, bundles, signal) {
    throwIfDeploymentExportAborted(signal);
    ...
    if (!indexedAsset)
        assertAuthorizedDeploymentSourceFile(sourcePath, '模型资源');
    ...
    if (reference.asset.scriptAssets !== undefined) {
        ...
        for (const script of scripts) {
            ...
            if (!scriptBundle.copyCompleteDirectory)
                assertAuthorizedDeploymentSourceFile(scriptPath, '模型脚本');
...
async function resolveModelBundle(sourcePath, packagePathHint, indexedAsset, projectContext, bundles, signal) {
    throwIfDeploymentExportAborted(signal);
    ...
    if (packagePathHint) {
        if (!isPathInsideOrEqual(packagePathHint, sourcePath))
            throw new Error('模型文件不在场景声明的 packagePath 内。');
...
async function resolveEnvironmentReference(reference, projectContext, bundles, signal) {
    throwIfDeploymentExportAborted(signal);
    ...
    if (!indexedAsset)
        assertAuthorizedDeploymentSourceFile(firstVariantPath, '环境变体');
    let packageRoot;
    let copyCompleteDirectory;
    if (indexedAsset?.packagePath) {
        await assertProjectPackageInsideRoot(indexedAsset.packagePath, projectContext);
    ...
    for (const variant of reference.variants) {
        ...
        if (!indexedAsset && !bundle.copyCompleteDirectory)
            assertAuthorizedDeploymentSourceFile(sourcePath, '环境变体');
        if (!isPathInsideOrEqual(bundle.sourceRoot, sourcePath))
            throw new Error('环境变体路径逃逸环境包根目录。');
        await addModelFileAndDependencies(bundle, sourcePath, signal);
        variantPaths.push({
            variant,
            sourcePath,
...
async function resolveCadReference(reference, bundles, signal) {
    throwIfDeploymentExportAborted(signal);
...
async function addModelFileAndDependencies(bundle, sourcePath, signal) {
    if (!isPathInsideOrEqual(bundle.sourceRoot, sourcePath))
        throw new Error('模型文件路径逃逸模型包根目录。');
    addExplicitFileToBundle(bundle, sourcePath);
    ...
    try {
        for (const dependencyPath of await dependencyPromise)
            addExplicitFileToBundle(bundle, dependencyPath);
    }
    catch (error) {
        if (bundle.gltfDependencyCache.get(sourceKey) === dependencyPromise)
            bundle.gltfDependencyCache.delete(sourceKey);
        throw error;
    }
}
/** 安全读取 glTF JSON 中的本地 URI，并拒绝远程依赖与包根逃逸。 */
async function readGltfDependencies(gltfPath, packageRoot, signal) {
    throwIfDeploymentExportAborted(signal);
    const stat = await fs.lstat(gltfPath);
    if (stat.isSymbolicLink() || !stat.isFile())
        throw new Error('glTF 主文件不是安全的普通文件。');
    if (stat.size > MAX_GLTF_JSON_BYTES)
        throw new Error('glTF JSON 超过安全解析上限。');
    let document;
    try {
        document = JSON.parse(await fs.readFile(gltfPath, 'utf8'));
    }
    catch {
    ...
    for (const uri of uris) {
        ...
        if (!isPathInsideOrEqual(packageRoot, dependencyPath))
            throw new Error('glTF 资源 URI 逃逸模型包根目录。');
        dependencies.add(dependencyPath);
    }
    return [...dependencies];
}
/** 从 glTF buffers/images 数组中收集非空 uri 字段。 */
function collectUriValues(value, target, label) {
    if (value === undefined)
        return;
    if (!Array.isArray(value))
        throw new Error(`${label} 格式不正确。`);
    for (const entry of value) {
        if (!isPlainObject(entry) || entry.uri === undefined)
            continue;
        if (typeof entry.uri !== 'string' || !entry.uri)
            throw new Error(`${label} URI 格式不正确。`);
        target.push(entry.uri);
    }
}
/** 建立或复用同一类别、同一资源根对应的复制包。 */
function getOrCreateBundle(bundles, category, sourceRoot, copyCompleteDirectory) {
    const normalizedRoot = path.resolve(sourceRoot);
    const key = `${category}:${toLocalPathKey(normalizedRoot)}`;
    const existing = bundles.get(key);
    if (existing) {
        if (copyCompleteDirectory)
            existing.copyCompleteDirectory = true;
        return existing;
    }
    ...
    const bundle = {
        ...
        explicitRelativePaths: new Set(),
        gltfDependencyCache: new Map(),
...
function addExplicitFileToBundle(bundle, sourcePath) {
...
async function createAssetCopyPlan(bundles, forbiddenOutputPaths, signal, onStatus, warnings) {
    const assetFiles = [];
    ...
    for (const [index, bundle] of bundles.entries()) {
        ...
        const files = await scanSafeSourceRoot(bundle.sourceRoot, bundle.copyCompleteDirectory ? null : bundle.explicitRelativePaths, forbiddenOutputPaths, signal);
        ...
        for (const file of files) {
            ...
            if (destinationKeys.has(destinationKey))
                throw new Error(`导出资源目标冲突：${destinationRelativePath}`);
...
function auditDeploymentPackageFiles(bundle, files, warnings) {
    if (!bundle.copyCompleteDirectory || (bundle.category !== 'models' && bundle.category !== 'environments'))
        return;
    ...
    const examples = [];
    for (const file of files) {
        if (!isKnownNonRuntimeDeploymentFile(file.relativePath))
            continue;
        nonRuntimeFileCount += 1;
        if (examples.length < 8)
            examples.push(toDeploymentPath(file.relativePath));
    }
...
function isExplicitSecretDeploymentFile(relativePath) {
    const normalizedPath = toDeploymentPath(relativePath).toLowerCase();
    const pathSegments = normalizedPath.split('/').filter(Boolean);
    const fileName = pathSegments[pathSegments.length - 1] ?? '';
    if (pathSegments.some((segment) => SECRET_DEPLOYMENT_DIRECTORY_NAMES.has(segment)))
        return true;
    if (SECRET_DEPLOYMENT_FILE_NAMES.has(fileName) || fileName.startsWith('.env.'))
        return true;
    if (/^service-account.*\.json$/i.test(fileName))
        return true;
    return SECRET_DEPLOYMENT_FILE_EXTENSIONS.has(path.posix.extname(fileName));
}
/** 判断相对路径是否属于常见设计源文件、备份、日志或依赖目录。 */
function isKnownNonRuntimeDeploymentFile(relativePath) {
    const normalizedPath = toDeploymentPath(relativePath).toLowerCase();
    const pathSegments = normalizedPath.split('/').filter(Boolean);
    if (pathSegments.some((segment) => NON_RUNTIME_DEPLOYMENT_DIRECTORY_NAMES.has(segment)))
        return true;
    const fileName = pathSegments[pathSegments.length - 1] ?? '';
    return NON_RUNTIME_DEPLOYMENT_FILE_EXTENSIONS.has(path.posix.extname(fileName));
}
/** 将普通模型、生成器目标及脚本路径改写为部署虚拟 URL。 */
function rewriteModelReferences(references, sourceUrlMap) {
...
function rewriteEnvironmentReferences(references, sourceUrlMap) {
    for (const reference of references) {
        ...
        for (const { variant, sourcePath, originalSourceUrl } of reference.variantPaths) {
            ...
            if (reference.originalActiveVariantUrl && (reference.originalActiveVariantUrl === originalSourceUrl
...
function rewriteCadReferences(references, sourceUrlMap) {
    for (const reference of references) {
...
function requireMappedUrl(sourceUrlMap, sourcePath, label) {
    const logicalUrl = sourceUrlMap.get(toLocalPathKey(sourcePath));
    if (!logicalUrl)
        throw new Error(`${label}未进入导出资源清单。`);
    return logicalUrl;
}
/** 判断旧 activeVariantUrl 是否指向给定本地文件。 */
function localReferenceMatches(reference, sourcePath) {
    try {
...
function findProjectAsset(assets, sourcePath, packagePathHint, libraryKind) {
    const sourceKey = toLocalPathKey(sourcePath);
    const packageKey = packagePathHint ? toLocalPathKey(packagePathHint) : null;
    const candidates = assets.filter((asset) => {
        if (asset.libraryKind !== libraryKind)
            return false;
        if (packageKey && asset.packagePath && toLocalPathKey(asset.packagePath) === packageKey)
            return true;
        if (toLocalPathKey(asset.path) === sourceKey)
            return true;
...
async function assertProjectPackageInsideRoot(packagePath, projectContext) {
    if (!projectContext)
        throw new Error('项目资产索引上下文缺失。');
    if (!isPathInsideOrEqual(projectContext.projectRoot, packagePath))
        throw new Error('项目资产包路径逃逸当前项目目录。');
...
function assertAuthorizedDeploymentSourceFile(filePath, label) {
    if (!isAuthorizedAssetFile(filePath))
        throw new Error(`${label}未经过当前会话的资产选择或项目授权，已拒绝读取。`);
}
/** 只有用户授权过的目录或带相邻 meta.json 的包根才允许完整复制。 */
async function isTrustedCompletePackageRoot(directoryPath) {
...
async function hasAdjacentPackageMetadata(directoryPath) {
    const metadataPath = path.join(directoryPath, 'meta.json');
    const stat = await lstatIfExists(metadataPath);
    if (!stat)
        return false;
    if (stat.isSymbolicLink())
        throw new Error('模型包 meta.json 不能是符号链接或 Junction。');
    if (!stat.isFile())
        throw new Error('模型包 meta.json 不是普通文件。');
    return true;
}
/** 优先从 sourcePath、其次从 sourceUrl 解出受支持的本地绝对路径。 */
function resolveLocalAssetPath(sourcePathValue, sourceUrlValue, label) {
    const sourcePath = typeof sourcePathValue === 'string' ? sourcePathValue.trim() : '';
    if (sourcePath) {
        if (sourcePath.startsWith(EDITOR_ASSET_URL_PREFIX))
            return decodeAssetUrl(sourcePath);
    ...
    if (sourceUrl.startsWith(EDITOR_ASSET_URL_PREFIX))
        return decodeAssetUrl(sourceUrl);
    throw new Error(`${label}缺少可解析的本地绝对路径。`);
}
/** 读取可选 packagePath；存在时必须能够解析为本地绝对路径。 */
function readOptionalLocalPath(value, label) {
...
function assertModelFileExtension(filePath, label) {
    if (!MODEL_EXTENSIONS.has(path.extname(filePath).toLowerCase()))
        throw new Error(`${label}必须是 .glb 或 .gltf 文件。`);
}
/** 根据资源类别和扩展名生成资产清单 kind。 */
function inferAssetKind(category, relativePath) {
    const lowerPath = relativePath.toLowerCase();
    const extension = path.extname(lowerPath);
    if (lowerPath.endsWith(SCRIPT_EXTENSION) || ['.ts', '.js', '.mjs', '.cjs'].includes(extension))
        return 'script';
    if (['.png', '.jpg', '.jpeg', '.webp', '.gif', '.bmp', '.dds', '.ktx', '.ktx2', '.hdr', '.env', '.exr'].includes(extension))
        return 'texture';
    if (['.bin', '.wasm'].includes(extension))
        return 'buffer';
    if (extension === '.json')
        return 'metadata';
    if (extension === '.dxf' || category === 'cad')
        return 'cad';
    if (MODEL_EXTENSIONS.has(extension))
        return category === 'environments' ? 'environment' : 'model';
    return 'asset';
}
/** 为清单文件生成不含本机路径的 editor-asset 虚拟 URL。 */
function createVirtualAssetUrl(destinationRelativePath) {
...
function createVirtualDirectoryUrl(destinationRoot) {
...
function createRuntimeMqttConfig(source, warnings) {
    ...
    if (address && !safeAddress)
        warnings.push('原场景 MQTT 地址不适合浏览器部署，runtime-config 已清空地址并保持禁用。');
    if (requestedEnabled && !enabled && safeAddress)
        warnings.push('原场景 MQTT 已启用但缺少有效订阅，runtime-config 已保持禁用。');
...
function createDisabledSceneMqttConfig() {
    return {
        enabled: false,
        ip: '',
        address: '',
...
function createRuntimeConfig(exportName, mqtt) {
    return {
        version: 1,
        page: {
            title: exportName,
...
function sanitizeMqttSubscriptions(value) {
    if (!Array.isArray(value))
        return [];
    const subscriptions = [];
    for (const entry of value.slice(0, 32)) {
        ...
        const subscription = { topic, qos: entry.qos === 1 ? 1 : 0, adapter };
        subscriptions.push(subscription);
    }
    return subscriptions;
}
/** 只复制 epv/json-path 适配器的公开配置字段。 */
function sanitizeMqttAdapter(value) {
    ...
    if (value.kind !== 'json-path')
        return null;
    const fields = {};
    if (isPlainObject(value.fields)) {
        for (const [key, fieldPath] of Object.entries(value.fields).slice(0, 128)) {
            if (typeof fieldPath === 'string' && key.length <= 128 && fieldPath.length <= 256)
                fields[key] = fieldPath;
        }
    }
...
function isSafeBrowserMqttAddress(address) {
    try {
...
function removeOptionalEditorOnlyUrls(value) {
    if (Array.isArray(value)) {
        for (const item of value)
            removeOptionalEditorOnlyUrls(item);
        return;
    }
    if (!isPlainObject(value))
        return;
    ...
    for (const child of Object.values(value))
        removeOptionalEditorOnlyUrls(child);
}
/** 递归确认 scene.json 中不存在 Windows、UNC、file:// 或绝对 editor-asset 本机路径。 */
function assertNoLocalMachinePaths(value, location = '$', seen = { count: 0 }) {
    seen.count += 1;
    if (seen.count > 1_000_000)
        throw new Error('场景结构过大，无法完成本机路径审计。');
    if (typeof value === 'string') {
    ...
    if (isPlainObject(value)) {
        for (const [key, child] of Object.entries(value))
            assertNoLocalMachinePaths(child, `${location}.${key}`, seen);
    }
}
/** 生成部署包内 README，说明运行方式与安全边界。 */
function createDeploymentReadme(exportName) {
