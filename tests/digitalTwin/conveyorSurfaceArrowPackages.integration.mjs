...
async function run() {
  ...
  try {
    ...
    const { serializeScene, deserializeScene } = await loadSceneSerializer();
    ...
    for (const entity of Object.values(zero.entities)) {
      ...
      if (arrows) { arrows.speed = 0; arrows.opacity = 0; }
    }
    ...
    ].map(fixture => ({ ...fixture, content: serializeScene(fixture.scene) }));
    ...
    assert.equal(source.sceneCount, 3);
    ...
    for (const fixture of fixtures) {
      ...
      assert.deepEqual(snapshot(deserializeScene(content)), snapshot(fixture.scene), fixture.name + ' SOURCE 重开');
      ...
      if (fixture.name !== 'main.scene.json') {
        ...
        assert.deepEqual(snapshot(deserializeScene(variantContent)), snapshot(fixture.scene), fixture.name + ' DIST 重开');
      }
    }
    ...
    assert.deepEqual(snapshot(deserializeScene(distContent)), snapshot(document.scene), 'DIST 重开');
    ...
    const viewerRoot = path.join(output, 'viewer-' + Date.now());
    ...
    await writeFile(path.join(output, 'packages-result.json'), JSON.stringify({
      ok: true, viewerRoot, sourceScenes: 3, distScenes: 3, conveyors: snapshot(document.scene).length,
      sourceBytes: source.fileSize, distBytes: dist.fileSize, distFiles: dist.fileCount,
      checks: ['source-main', 'source-zero-speed-opacity', 'source-legacy-absent', 'source-reopen', 'dist-main-zero-legacy-and-reopen', 'dist-model-script', 'source-not-mutated'],
    }, null, 2));
    console.log('PASS: 输送线表面箭头 SOURCE 三场景、DIST、零值、旧场景缺省与源文件不变。');
    code = 0;
