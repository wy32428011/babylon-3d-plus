...
const { app } = electron;
const root = await mkdtemp(path.join(tmpdir(), 'light-wall-packages-'));
...
async function cleanup() {
  ...
  if (!relative || relative.startsWith('..') || path.isAbsolute(relative)
    || !path.basename(root).startsWith('light-wall-packages-')) throw new Error('拒绝清理测试临时目录之外的路径');
  await rm(root, { recursive: true, force: true });
}
const deadline = setTimeout(() => {
  console.error('光墙围栏双包验证超时');
...
function walls(document) {
  return Object.values(document.scene.entities)
    .filter(entity => entity.components.poiEffect?.effectKind === 'light-wall-fence')
...
async function run() {
  ...
  try {
    ...
    const content = await readFile('output/light-wall-fence/scene.scene.json', 'utf8');
    const source = JSON.parse(content);
    assert.equal(walls(source).length, 1, '先运行编辑器集成测试生成围栏场景');
    ...
    const secondWall = Object.values(second.scene.entities).find(entity => entity.components.poiEffect?.effectKind === 'light-wall-fence');
    secondWall.components.poiEffect.speed = 0;
    secondWall.components.poiEffect.lightWall.opacity = 0;
    ...
    const sourcePackage = await buildDigitalTwinSourcePackage({
      ...
      manifest: { projectId: '123', projectName: '光墙围栏验收', editorProjectId: null, baseVersionId: null, resourceRevision: '1' },
    ...
    const distPackage = await buildDigitalTwinDistPackage({
      projectId: '123', publishName: '光墙围栏验收', sceneContent: sourcePackage.entrySceneContent,
    ...
    const viewerRoot = path.resolve('output/light-wall-fence/viewer');
    ...
    await writeFile('output/light-wall-fence/packages-result.json', JSON.stringify({
      ok: true, sourceScenes: 2, entryWalls: walls(source).length, contourPoints: walls(source)[0].effect.lightWall.points.length,
      distFiles: distPackage.fileCount, viewerRoot,
      checks: ['source-entry', 'source-second-zero-values', 'dist-entry', 'source-not-mutated'],
    }, null, 2));
    console.log('SOURCE 双场景 ZIP 与 DIST 实际 ZIP：颜色、高度、透明度、速度、凹轮廓和 Transform 保留验证通过');
    code = 0;
  } catch (error) {
    console.error(error);
  } finally {
