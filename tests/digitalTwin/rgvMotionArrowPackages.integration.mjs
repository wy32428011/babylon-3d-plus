...
const root = await mkdtemp(path.resolve('node_modules/.rgv-motion-arrow-packages-'));
...
function cleanup() {
  return cleanupPromise ??= (async () => {
    ...
    assert.ok(path.basename(root).startsWith('.rgv-motion-arrow-packages-'));
    await rm(root, { recursive: true, force: true });
  })();
}
const deadline = setTimeout(() => {
  console.error('RGV箭头 SOURCE/DIST 验证超时'); abortController.abort();
  void cleanup().catch(console.error).finally(() => app.exit(1));
}, 180000);
const snapshot = scene => Object.values(scene.entities).filter(entity => entity.components.telemetryBinding?.deviceType === 'rgv')
...
async function run() {
  ...
  try {
    ...
    const rgvSnapshots = snapshot(document.scene);
    assert.ok(rgvSnapshots.length > 0, '输入必须包含 RGV 验收实体');
    assert.ok(rgvSnapshots.every(value => value.binding.rgvMotionArrows?.enabled));
    for (const value of rgvSnapshots) {
      const travel = value.binding.rgvMotionArrows.channels.travel;
      assert.equal(travel.face, 'top', '持久配置必须位于轨道正上方');
      assert.equal(travel.offsetAcross, 0, '持久配置必须居中');
      assert.equal(travel.offsetAlong, 0, '持久配置必须覆盖轨道全长');
      assert.equal(travel.length, 0, '持久配置必须自动匹配轨道长度');
    }
    ...
    const { serializeScene, deserializeScene, createDefaultRgvMotionArrowsConfig } = await import(pathToFileURL(path.join(root, 'modules/serializer.mjs')).href);
    assert.equal(createDefaultRgvMotionArrowsConfig().enabled, false, 'RGV旧场景不应自动新增箭头');
    const projectRoot = path.join(root, 'project'), modelRoot = path.join(projectRoot, 'Assets/Models/arrow-rgv');
    ...
    const variants = ['main', 'disabled', 'legacy', 'zero'].map(key => {
      const scene = structuredClone(document.scene); scene.name = 'RGV箭头-' + key;
      for (const entity of Object.values(scene.entities)) {
        const binding = entity.components.telemetryBinding;
        if (binding?.deviceType !== 'rgv') continue;
        if (key === 'legacy') delete binding.rgvMotionArrows;
        if (key === 'disabled') binding.rgvMotionArrows.enabled = false;
        if (key === 'zero') Object.assign(binding.rgvMotionArrows, { speed: 0, opacity: 0, intensity: 0, breathingStrength: 0 });
      }
      const content = serializeScene(scene);
      return { key, scene, content, expected: snapshot(deserializeScene(content)) };
    });
    assert.ok(variants.find(item => item.key === 'legacy').expected.every(value => value.binding.rgvMotionArrows === undefined));
    ...
    const source = await buildDigitalTwinSourcePackage({ projectRoot, sharedResourcesRoot: path.join(root, 'shared'), entrySceneFilePath: path.join(scenesRoot, 'main.scene.json'),
      ...
      manifest: { projectId: '123', projectName: 'RGV运动箭头验收', editorProjectId: null, baseVersionId: null, resourceRevision: '1' },
    ...
    for (const variant of variants) {
      ...
      assert.ok(archive.files.some(file => /arrow-rgv\.glb$/.test(file.path)));
      assert.ok(archive.files.some(file => /arrow-rgv\.model\.(ts|js)$/.test(file.path)));
    ...
    await writeFile(path.join(output, 'packages-result.json'), JSON.stringify({ ok: true, viewers, sourceScenes: variants.length, distScenes: variants.length,
      checks: ['three-channel-config', 'track-top-centered-full-length', 'SOURCE-DIST-reopen', 'legacy-default-off', 'explicit-disabled', 'zero-values', 'model-and-motion-script', 'source-not-mutated'] }, null, 2));
    console.log('PASS: RGV箭头 SOURCE/DIST 四场景配置/重开/旧场景关闭/显式关闭/零值/模型脚本'); code = 0;
