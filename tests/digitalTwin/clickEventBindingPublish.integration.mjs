...
async function run() {
  ...
  try {
    ...
    binding.components.clickEventBinding = {
      ...
      events: [
        { id: 'click', eventType: 'click', effects: ['highlight', 'focus', 'show-chart'], chart: { id: 'data-platform-screen:project-001:screen-model', projectId: 'project-001', screenId: 'screen-model', name: '模型图表' } },
        { id: 'cell', eventType: 'click-cell', effects: ['highlight', 'focus', 'show-chart'], chart: { id: 'data-platform-screen:project-001:screen-cell', projectId: 'project-001', screenId: 'screen-cell', name: '货格图表' } },
      ],
    };
    ...
    function assertClick(loaded, label) {
      ...
      assert.deepEqual(shownScreens[1], { projectId: 'project-001', screenId: 'screen-cell' }, `${label}必须请求宿主展示货格绑定大屏`);
      ...
      assert.equal(shownScreens.length, 2, '点击空白不能触发大屏切换');
    }
    ...
    for (const event of legacy.scene.entities[binding.id].components.clickEventBinding.events) {
      delete event.chart.projectId;
      delete event.chart.screenId;
    }
