...
test('46 类特效 V2 的全部字段支持 Store、撤销重做、保存重开和参数副本隔离', async () => {
  assert.equal(VISIBLE_POI_EFFECT_DEFINITIONS.length, 46);
  const target = createMeshEntity('cube'); target.name = '仅用于特效绑定的测试建筑';
  const scene = createEmptySceneDocument('全部 46 类特效 V2'); append(scene, [target]); store.setState({ scene });
