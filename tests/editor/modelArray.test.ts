...
test('阵列数量和净间距必须满足共享提交边界', () => {
  assert.match(getEntityArrayParameterError(0, 0) ?? '', /1-1000/);
  assert.match(getEntityArrayParameterError(1001, 0) ?? '', /1-1000/);
  ...
  assert.equal(getEntityArrayParameterError(1000, 0), null);
});
...
test('拖动距离按半跨度阈值、正负方向和上限换算副本数量', () => {
  ...
  assert.equal(calculateModelArraySignedCopyCount(500, 1), 500);
  assert.equal(calculateModelArraySignedCopyCount(5000, 1), 1000);
...
test('已有模型矩阵阵列项的名称继续参与冲突校验', () => {
  const source = createModelEntity('source', 'Source', 'DEV9');
  source.components.modelArray = {
    items: [{
      id: 'model-array-name-conflict',
      name: 'Source1',
      assetCode: 'OTHER1',
      offset: { x: 1, y: 0, z: 0 },
    }],
  ...
  assert.match(
    getEntityArrayIdentifierError(createScene(source), ['source'], 1, '') ?? '',
    /对象名称“Source1”已存在/,
  );
});

test('已有模型矩阵阵列项的资产编号继续参与冲突校验', () => {
  const source = createModelEntity('source', 'Source', 'DEV9');
  source.components.modelArray = {
    items: [{
      id: 'model-array-asset-conflict',
      name: 'Existing Array Item',
      assetCode: 'DEV10',
      offset: { x: 1, y: 0, z: 0 },
    }],
  };
