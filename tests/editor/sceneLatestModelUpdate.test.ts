...
after(() => vite.close());
const { applySceneModelUpdates } = await vite.ssrLoadModule('/src/editor/assets/applySceneModelUpdates.ts');
const { createEmptySceneDocument, createModelEntity, createModelGeneratorEntity } = await vite.ssrLoadModule('/src/editor/model/SceneDocument.ts');
...
test('场景更新可一次撤销，渲染失败回退，旧会话和旧事务不能修改新场景', async () => {
  ...
  assert.equal(useEditorStore.getState().scene, before.scene);
  assert.equal(useEditorStore.getState().history, before.history);
  assert.equal(before.commitLatestSceneResources(before.sceneSessionId, before.scene, updated), true);
