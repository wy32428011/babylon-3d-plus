...
test('真实store场景提交边界丢弃取消后的loadSceneFile结果及后续同步', async () => {
  ...
  let commits = 0;
  let syncs = 0;
  let committed: { environmentStartupRelinkSessionId?: string } = {};
  ...
  resolve({ canceled: false, content: '{"name":"current"}', filePath: 'current.scene.json' });
  assert.equal(await normal, true);
  assert.equal(commits, 1);
  assert.equal(syncs, 3);
  assert.equal(committed.environmentStartupRelinkSessionId, undefined);
